...
router.get('/categories', (req, res) => {
  const categories = [
    ...
    { id: 'administrative', name: 'ადმინისტრაციული', icon: '📄', pricePerMinute: 2.5 },
...
router.get('/:id/stats', async (req, res) => {
  try {
    ...
    const avgRating = calls.length > 0 
      ? calls.reduce((sum, c) => sum + (c.rating || 0), 0) / calls.filter(c => c.rating).length
      : 0;
...
function getPriceForCategory(category) {
  const prices = { general: 2, administrative: 2.5, business: 3, medical: 4, legal: 4 };
