...
export class DatabaseStorage implements IStorage {
  ...
  async getOnlineTranslators(): Promise<Translator[]> {
    return db
      .select()
      .from(translators)
      .where(eq(translators.isOnline, true))
      .orderBy(desc(translators.rating));
  }

  async createTranslator(translator: InsertTranslator): Promise<Translator> {
    const [newTranslator] = await db
      .insert(translators)
      .values(translator)
      .returning();
  ...
  async updateTranslatorStats(
    ...
    await db
      .update(translators)
      .set({ rating, totalCalls, totalMinutes })
      .where(eq(translators.id, id));
  }
